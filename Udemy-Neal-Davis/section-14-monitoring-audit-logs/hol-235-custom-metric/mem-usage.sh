...
aws cloudwatch put-metric-data --region eu-west-2 --namespace "Custom/Memory" --metric-name "MemUsage" --value "$MEMORY_USAGE" --unit "Percent" --dimensions "InstanceName=MemoryUsageInstance,InstanceId=$INSTANCE_ID"

echo "Memory usage: $MEMORY_USAGE% (Instance ID: $INSTANCE_ID)" >> memory-usage.log