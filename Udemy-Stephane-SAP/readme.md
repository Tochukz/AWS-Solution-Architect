# Udemy Course - AWS Certified Solution Architect Professional

__By Stephane Maarek__

__Content__  

Section | Pg  | Title
--------|-----|--------------
1       | 3   | Introduction
2       | 3   | Slides Download  
3       | 9   | Identity & Federation
4       | 80  | Security
5       | 174 | Compute and Load Balancing
6       | 320 | Storage

### Resources
[Slide and Code Download](https://courses.datacumulus.com/downloads/certified-solutions-architect-professional-m0v/)

### Demo
#### Section 3
Tested  | Demo
--------|---------
 Yes    | 4 Permission Boundary
 No     | 4 Restricting Tags Policies
 Yes    | 5 Policy Generation
 Yes    | 5 Policy Validator
 No     | 13 Resource Sharing with RAM

#### Section 4
Tested |  Demo
-------|-------------
 No    | 17 CloudTrail Security Alert
 No    | 17 Organizational Trail
 No    | 18 CloudHSM Custom Key
 No    | 18 Imported Key Material
 No    | 18 Multi-Region Key
 Partially | 19 Advanced Parameters
 No    | 20 Cross Account Secret Sharing
 No    | 20 Secret Manager with RDS
 No    | 25 CloudHSM SSL Offloading
 No    | 25 SSL on EC2 Instances
 No    | 32 Enhanced CloudFront Origin Security
 No    | Extra-32 - CloudFront ABL Custom Header
 No    | 40 EC2 Instance Connect
 No    | Hands-on 29 - Multi Region Access Point

#### Section 5
Tested | Demo  
-------|--------------
No     | 46 Auto Scaling Load Balancer
No     | 49 ECS Batch Processing
No     | 49 ECS with Fargate Spot
No     | 49 ECS with Spot Instance  
No     | 50 ECR Basic Scanning
No     | 50 ECR Cross Region Replication  
No     | 50 ECR Enhanced Scanning
No     | 52 App Runner Multi Region
No     | 58 API Gateway Authentication with Congnito
No     | 58 API Gateway Authentication with IAM
No     | 58 API Gateway to S3 Integration
No     | 59 API Gateway Websocket
No     | 59 Private API Gateway
No     | 61 Health Check Private Hosted Zones
No     | 61 Private Hosted Zone
No     | 64 Global Accelerator
No     | 65 API Gateway to SQS
No     | 68 Local Zones 
